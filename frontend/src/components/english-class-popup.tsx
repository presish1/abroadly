...
export function EnglishClassPopup() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      // Only show if they haven't seen it recently (could use localStorage, keeping it simple for now)
      setShow(true);
    ...
    <>
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-[90%] max-w-sm overflow-hidden rounded-[24px] bg-white p-6 shadow-[0_20px_60px_rgba(0,0,0,0.15)] border border-slate-200 animate-[abFadeUp_0.5s_ease-out]">
        ...
        <button
          ...
          className="w-full rounded-full bg-[#0044FF] py-3 text-sm font-bold text-white shadow-md transition hover:-translate-y-0.5 hover:bg-blue-600"
        >
          Claim my free class
        </button>
      </div>
