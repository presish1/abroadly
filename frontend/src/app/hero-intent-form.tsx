 ...
 * Light / flat styling — sits on the off-white editorial hero. */
...
const selectClass =
  "ab-focus cursor-pointer appearance-none rounded-xl border border-[var(--ab-line)] bg-white py-2.5 pl-4 pr-10 text-[16px] font-bold tracking-[-0.01em] text-[var(--ab-ink)] transition hover:border-[#CFC9BD] focus:border-[var(--ab-brand)] sm:text-[17px]";
...
export function HeroIntentForm() {
  ...
  return (
    <div className="flex w-full flex-col items-center gap-6">
      {/* selector sentence */}
      <div className="flex flex-wrap items-center justify-center gap-x-2.5 gap-y-3 text-[17px] font-semibold text-[var(--ab-ink-soft)] sm:text-[18px]">
      ...
      {/* primary CTA — Google-branded, free, flat */}
      <button
        type="button"
        onClick={start}
        className="ab-focus inline-flex items-center gap-2.5 rounded-xl bg-[var(--ab-ink)] py-3 pl-3 pr-6 text-[15px] font-extrabold text-white transition hover:-translate-y-0.5 hover:bg-black active:translate-y-0"
      >
        <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-white">
          <GoogleMark />
        </span>
        Find my fit
        <span className="font-semibold text-white/60">· free</span>
      </button>
    </div>
  );
}
