...
/* Quiet, flat trust strip — real universities our guidance covers.
 * Grayscale favicons (monogram fallback), warming to colour on hover. */
...
function UniItem({ name, domain }: { name: string; domain: string }) {
  ...
  return (
    <span className="group inline-flex items-center gap-2 opacity-80 transition hover:opacity-100">
      {failed ? (
        <span className="flex h-[22px] w-[22px] items-center justify-center rounded-md bg-[var(--ab-paper-2)] text-[10px] font-extrabold text-[var(--ab-muted)]">
          {name[0]}
        </span>
      ) : (
        <img
